...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
	  ...
	  for (int t = 0; t < N - 1; t++) {
		  fg[0] += 1 * pow(vars[delta_start + t], 2);
		  fg[0] += pow(vars[a_start + t], 2);
	  }
	  for (int t = 0; t < N - 2; t++) {
		  fg[0] += 2000 * pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);
		  fg[0] += 1 * pow(vars[a_start + t + 1] - vars[a_start + t], 2);
	  }
...
vector<vector<double>> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  std::cout << "n_vars: " << n_vars << " delta_start: " << delta_start << std::endl;
  ...
  //std::cout << "Computing fg_eval" << std::endl;
  ...
  actuators.push_back(solution.x[delta_start + 1]);
  actuators.push_back(solution.x[a_start + 1]);
  for (int i = 0; i < N; i++)
  {
